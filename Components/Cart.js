let kosar = !localStorage.getItem("kosarbaRakottTermekek") ? [] : JSON.parse(localStorage.getItem("kosarbaRakottTermekek"));
let cartAmount = kosar.length;
...
function inputChange(id, q) {
    const item = kosar.find(item => item.id === id);
    if (item) {
        item.quantity = q;
    }
    SaveTheCart();
...
function removeThisItem(id, localTermekek) {
    const index = kosar.findIndex(item => item.id === id);
    if (index !== -1) {
        kosar.splice(index, 1); // Item törlése a kosárból
